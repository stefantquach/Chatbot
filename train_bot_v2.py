...
encoder_lstm = layers.LSTM(200, return_sequences=True, name='encoder_lstm')
encoder_output = encoder_lstm(embedding(encoder_input))

decoder_input = layers.Input(shape=(None, ), name='decoder_input')
decoder_lstm = layers.LSTM(200, return_sequences=True, name='decoder_lstm')
decoder_output = decoder_lstm(embedding(decoder_input))

decoder_merge = layers.concatenate([encoder_output, decoder_output], axis=1, name='decoder_merge')
...
output = decoder_dense(decoder_hidden(decoder_merge))

train_model = models.Model([encoder_input, decoder_input], output)
train_model.compile(optimizer=tf.keras.optimizers.Adam(), loss='categorical_crossentropy')
